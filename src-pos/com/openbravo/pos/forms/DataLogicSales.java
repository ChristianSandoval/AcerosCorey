...
public final boolean getCustomerByTarjeta(String id)
    throws BasicException
    {
      return new PreparedSentence(this.s, "SELECT SEARCHKEY FROM CUSTOMERS WHERE SEARCHKEY = ?", SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE).find(id) != null;
    }

public CustomerInfoExt findCustomerExtById(String id)
    throws BasicException
    {
      return (CustomerInfoExt)new PreparedSentence(this.s, "SELECT ID, TAXID, SEARCHKEY, NAME, CARD, TAXCATEGORY, NOTES, MAXDEBT, VISIBLE, CURDATE, CURDEBT, FIRSTNAME, LASTNAME, EMAIL, PHONE, PHONE2, FAX, ADDRESS, ADDRESS2, POSTAL, CITY, REGION, COUNTRY FROM CUSTOMERS WHERE ID = ?", SerializerWriteString.INSTANCE, new CustomerExtRead()).find(id);
    }
public CustomerInfoExt findCustomerExtByCard(String card)
    throws BasicException
    {
      return (CustomerInfoExt)new PreparedSentence(this.s, "SELECT ID, TAXID, SEARCHKEY, NAME, CARD, TAXCATEGORY, NOTES, MAXDEBT, VISIBLE, CURDATE, CURDEBT, FIRSTNAME, LASTNAME, EMAIL, PHONE, PHONE2, FAX, ADDRESS, ADDRESS2, POSTAL, CITY, REGION, COUNTRY FROM CUSTOMERS WHERE SEARCHKEY = ?", SerializerWriteString.INSTANCE, new CustomerExtRead()).find(card);
    }
    ...
    public final ProductInfoExt getProductInfo(String id) throws BasicException {
        return (ProductInfoExt) new PreparedSentence(s
            , "SELECT ID, REFERENCE, CODE, NAME, ISCOM, ISSCALE, PRICEBUY, PRICESELL, TAXCAT, CATEGORY, ATTRIBUTESET_ID, IMAGE, ATTRIBUTES, STOCKVOLUME " +
    ...
    public List<ProductInfoExt> getProductCatalog(String category) throws BasicException  {
        return new PreparedSentence(s
            , "SELECT P.ID, P.REFERENCE, P.CODE, P.NAME, P.ISCOM, P.ISSCALE, P.PRICEBUY, P.PRICESELL, P.TAXCAT, P.CATEGORY, P.ATTRIBUTESET_ID, P.IMAGE, P.ATTRIBUTES, P.STOCKVOLUME " +
    ...
    public SentenceList getProductListNormal() {
        return new StaticSentence(s
            , new QBFBuilder(
              "SELECT ID, REFERENCE, CODE, NAME, ISCOM, ISSCALE, PRICEBUY, PRICESELL, TAXCAT, CATEGORY, ATTRIBUTESET_ID, IMAGE, ATTRIBUTES,STOCKVOLUME " +
              "FROM PRODUCTS,PRODUCTS_CAT WHERE PRODUCTS.ID = PRODUCTS_CAT.PRODUCT AND ?(QBF_FILTER) ORDER BY PRICESELL", new String[] {"NAME", "PRICEBUY", "PRICESELL", "CATEGORY", "CODE"})
            , new SerializerWriteBasic(new Datas[] {Datas.OBJECT, Datas.STRING, Datas.OBJECT, Datas.DOUBLE, Datas.OBJECT, Datas.DOUBLE, Datas.OBJECT, Datas.STRING, Datas.OBJECT, Datas.STRING})
            , ProductInfoExt.getSerializerRead());
    }
    ...
    public CustomerInfoExt findCustomerExt(String card) throws BasicException {
        ...
        return (CustomerInfoExt) new PreparedSentence(s
                , "SELECT ID, TAXID, SEARCHKEY, NAME, CARD, TAXCATEGORY, NOTES, MAXDEBT, VISIBLE, CURDATE, CURDEBT" +
                  ...
                  " FROM CUSTOMERS WHERE SEARCHKEY = ? AND VISIBLE = " + s.DB.TRUE()
                , SerializerWriteString.INSTANCE
    ...
    public final void saveTicket(final TicketInfo ticket, final String location) throws BasicException {
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                // new ticket
                new PreparedSentence(s
                    , "INSERT INTO TICKETS (ID, TICKETTYPE, TICKETID, PERSON, CUSTOMER, TARJETA, NOMBRE, EMPRESA) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
                    , SerializerWriteParams.INSTANCE
                    ).exec(new DataParams() { public void writeValues() throws BasicException {
                        setString(1, ticket.getId());
                        ...
                        setString(6, ticket.getCustomerTarjeta());
                        setString(7, ticket.getCustomerName());
                        setString(8, ticket.getCustomerCompany());
                    }});
    ...
    public final void saveTicket3(final TicketInfo ticket, final String location) throws BasicException {
